...
var CameraSchema = exports_external.object({
  id: exports_external.string().describe("Unique identifier (slug)"),
  brand: exports_external.string(),
  model: exports_external.string(),
  sensor: exports_external.object({
    width_mm: exports_external.number().positive().describe("Sensor width in millimeters"),
    height_mm: exports_external.number().positive().describe("Sensor height in millimeters"),
    resolution_px_width: exports_external.number().int().positive().describe("Horizontal pixel count"),
    resolution_px_height: exports_external.number().int().positive().describe("Vertical pixel count"),
    pixel_pitch_um: exports_external.number().positive().optional().describe("Derived pixel pitch in micrometers")
  }),
  mount: exports_external.string(),
  weight_g: exports_external.number().positive()
});
var LensSchema = exports_external.object({
  id: exports_external.string().describe("Unique identifier (slug)"),
  brand: exports_external.string(),
  model: exports_external.string(),
  focal_length_mm: exports_external.number().positive(),
  max_aperture: exports_external.number().positive(),
  mount: exports_external.string(),
  weight_g: exports_external.number().positive()
});
...
var ROOT = path.join(__dirname2, "../../../");
var DATA_CAMERAS = path.join(ROOT, "data/cameras");
var OUTPUT_DIR = path.join(ROOT, "dist/api/v1");
var CAMERAS_OUTPUT_DIR = path.join(OUTPUT_DIR, "cameras");
async function compile() {
  ...
  for (const file of jsonFiles) {
    ...
    if (!camera.sensor.pixel_pitch_um) {
      ...
      console.log(`\u2728 Derived pixel_pitch_um for ${camera.id}: ${camera.sensor.pixel_pitch_um.toFixed(2)}\xB5m`);
    }
